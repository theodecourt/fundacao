...
def segundo_programa(idioma):
    if idioma == "Português":
        ...
        # Botão sempre visível para recalcular valores iniciais
        ...
        # Exibe os resultados, se já calculados
        ...
        # Solicita os dados de entrada do usuário
        Q = st.number_input("Enter the value of Q (tf):", min_value=0.0, value=100.0, key="Q")
        NSPT = st.number_input("Enter the value of NSPT:", min_value=0.0, value=10.0, key="NSPT")
        tipo_solo = st.selectbox("Select the type of soil (1, 2 or 3):", [1, 2, 3], key="tipo_solo")

        # Botão sempre visível para recalcular valores iniciais
        if st.button("Calculate initial values"):
            quc, q, Beq, area, s0 = calcular_valores(Q, NSPT, tipo_solo)
            # Salva os valores no session_state
            st.session_state["quc"] = quc
            st.session_state["q"] = q
        ...
        if "quc" in st.session_state:
            ...
            st.write(f"q (tf/m²): {st.session_state['q']}")
            st.write(f"Beq (m): {st.session_state['Beq']}")
            st.write(f"Initial Area (m²): {st.session_state['area']}")
            st.write(f"Initial s0 (mm): {st.session_state['s0']}")

            # Solicita o valor de s1 (mm)
            s1 = st.number_input("Enter the value of s1 (mm):", min_value=0.0, value=10.0)

            if st.button("Adjust q for s1"):
                # Calcula o novo q que resultaria em s0 = s1
                q_new = ajustar_q_para_s1(Q, s1, st.session_state["quc"], 1, 0.42)
                Beq_new = math.sqrt(Q / q_new)
                area_new = Beq_new ** 2
                ...
                # Exibe o resultado do novo q, Beq e área
                st.write(f"New q (tf/m²) for s0 = s1: {q_new}")
                st.write(f"New Beq (m) with adjusted q: {Beq_new}")
                st.write(f"New Area (m²) with adjusted Beq: {area_new}")
