...
def primeiro_programa(idioma):
    if idioma == "Português":
        ...
        if tabela is not None:
            ...
            pontos_tipos = []
            for i in range(num_regressoes):
                lin_in = st.number_input(f'Starting point of the regression {num_romanos[i+1]}:', min_value=1, max_value=len(tabela), value=1)
                lin_fim = st.number_input(f'Ending point of the regression {num_romanos[i+1]}:', min_value=lin_in, max_value=len(tabela), value=len(tabela))
